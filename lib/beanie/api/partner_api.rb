=begin
#Beanie ERP API

#An API specification for interacting with the Beanie ERP system

OpenAPI spec version: 0.8
Contact: dev@bean.ie
Generated by: https://github.com/swagger-api/swagger-codegen.git
Swagger Codegen version: 2.4.0-SNAPSHOT

=end

require 'uri'

module Beanie
  class PartnerApi
    attr_accessor :api_client

    def initialize(api_client = ApiClient.default)
      @api_client = api_client
    end
    # Creates a new partner in the system
    # @param partners Partner to add to the system
    # @param [Hash] opts the optional parameters
    # @return [Partner]
    def add_partner(partners, opts = {})
      data, _status_code, _headers = add_partner_with_http_info(partners, opts)
      data
    end

    # Creates a new partner in the system
    # @param partners Partner to add to the system
    # @param [Hash] opts the optional parameters
    # @return [Array<(Partner, Fixnum, Hash)>] Partner data, response status code and response headers
    def add_partner_with_http_info(partners, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: PartnerApi.add_partner ...'
      end
      # verify the required parameter 'partners' is set
      if @api_client.config.client_side_validation && partners.nil?
        fail ArgumentError, "Missing the required parameter 'partners' when calling PartnerApi.add_partner"
      end
      # resource path
      local_var_path = '/partners'

      # query parameters
      query_params = {}

      # header parameters
      header_params = {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json'])
      # HTTP header 'Content-Type'
      header_params['Content-Type'] = @api_client.select_header_content_type(['application/json'])

      # form parameters
      form_params = {}

      # http body (model)
      post_body = @api_client.object_to_http_body(partners)
      auth_names = ['api_key']
      data, status_code, headers = @api_client.call_api(:POST, local_var_path,
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => 'Partner')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: PartnerApi#add_partner\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
    # Find Partner by ID
    # Returns a single partner if the user has access
    # @param id ID of partner to fetch
    # @param [Hash] opts the optional parameters
    # @return [Customer]
    def find_partner_by_id(id, opts = {})
      data, _status_code, _headers = find_partner_by_id_with_http_info(id, opts)
      data
    end

    # Find Partner by ID
    # Returns a single partner if the user has access
    # @param id ID of partner to fetch
    # @param [Hash] opts the optional parameters
    # @return [Array<(Customer, Fixnum, Hash)>] Customer data, response status code and response headers
    def find_partner_by_id_with_http_info(id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: PartnerApi.find_partner_by_id ...'
      end
      # verify the required parameter 'id' is set
      if @api_client.config.client_side_validation && id.nil?
        fail ArgumentError, "Missing the required parameter 'id' when calling PartnerApi.find_partner_by_id"
      end
      # resource path
      local_var_path = '/partners/{id}'.sub('{' + 'id' + '}', id.to_s)

      # query parameters
      query_params = {}

      # header parameters
      header_params = {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json'])
      # HTTP header 'Content-Type'
      header_params['Content-Type'] = @api_client.select_header_content_type(['application/json'])

      # form parameters
      form_params = {}

      # http body (model)
      post_body = nil
      auth_names = ['api_key']
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => 'Customer')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: PartnerApi#find_partner_by_id\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
    # All partners
    # Returns all partners from the system that the user has access to
    # @param [Hash] opts the optional parameters
    # @option opts [Array<String>] :tags tags to filter by
    # @option opts [Integer] :limit Maximum number of results to return
    # @return [Array<Partner>]
    def find_partners(opts = {})
      data, _status_code, _headers = find_partners_with_http_info(opts)
      data
    end

    # All partners
    # Returns all partners from the system that the user has access to
    # @param [Hash] opts the optional parameters
    # @option opts [Array<String>] :tags tags to filter by
    # @option opts [Integer] :limit Maximum number of results to return
    # @return [Array<(Array<Partner>, Fixnum, Hash)>] Array<Partner> data, response status code and response headers
    def find_partners_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: PartnerApi.find_partners ...'
      end
      # resource path
      local_var_path = '/partners'

      # query parameters
      query_params = {}
      query_params[:'tags'] = @api_client.build_collection_param(opts[:'tags'], :csv) if !opts[:'tags'].nil?
      query_params[:'limit'] = opts[:'limit'] if !opts[:'limit'].nil?

      # header parameters
      header_params = {}
      # HTTP header 'Accept' (if needed)
      header_params['Accept'] = @api_client.select_header_accept(['application/json'])
      # HTTP header 'Content-Type'
      header_params['Content-Type'] = @api_client.select_header_content_type(['application/json'])

      # form parameters
      form_params = {}

      # http body (model)
      post_body = nil
      auth_names = ['api_key']
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
        :auth_names => auth_names,
        :return_type => 'Array<Partner>')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: PartnerApi#find_partners\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
end
